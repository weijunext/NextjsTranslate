...
你的项目命名为什么？my-app
你想使用 TypeScript 吗？不 / 是
你想使用 ESLint 吗？不 / 是
你想使用 Tailwind CSS 吗？不 / 是
你想使用 `src/` 目录吗？不 / 是
你想使用应用路由器吗？(推荐) 不 / 是
你想自定义默认导入别名 (@/*) 吗？不 / 是
你想配置哪个导入别名？@/*
...
对于新应用，我们推荐使用 [应用路由器](/docs/app)。这个路由器允许你使用 React 的最新功能，并且是基于社区反馈对 [页面路由器](/docs/pages) 的进化。
...
在 `app/layout.tsx` 内创建一个 [根布局](/docs/app/building-your-application/routing/pages-and-layouts#root-layout-required)，包含必需的 `<html>` 和 `<body>` 标签：
...
> **值得了解**：如果你忘记创建 `layout.tsx`，当运行开发服务器 `next dev` 时，Next.js 会自动创建这个文件。

了解更多关于 [使用应用路由器](/docs/app/building-your-application/routing/defining-routes)。

#### `pages` 目录（可选）

如果你更愿意使用页面路由器而不是应用路由器，你可以在项目的根目录创建一个 `pages/` 目录。
...
了解更多关于 [使用页面路由器](/docs/pages/building-your-application/routing/pages-and-layouts)。

> **值得了解**：虽然你可以在同一个项目中使用两种路由器，但 `app` 中的路由会优先于 `pages`。我们推荐在你的新项目中只使用一种路由器，以避免混淆。
